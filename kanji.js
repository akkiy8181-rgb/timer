  ...
  if (days.length === 0) {
    ...
    listEl.querySelectorAll('.day-test-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        startQuiz(buildQueue(day.items.map(it => ({ ...it, dayLabel: day.label })), 'sequential', 'all'), 'test');
      });
    });
...
function openQuizSetup() {
  ...
  if (days.length === 0) {
    ...
    wrap.innerHTML = days.map(day => `
      <label>
        <input type="checkbox" class="setup-day-check" value="${day.id}" checked>
...
function setupCanvas(targetText, showGuideGlyph) {
  ...
  const wrapWidth = Math.min(document.getElementById('quiz-canvas-wrap').clientWidth || 460, 460);
  let boxSize = Math.floor((wrapWidth - gap * (perRow - 1)) / perRow);
  boxSize = Math.max(80, Math.min(170, boxSize));
...
function setupEventListeners() {
  ...
  document.getElementById('single-add-btn').addEventListener('click', () => {
    ...
    pendingItems.push({ id: uid(), kanji, reading, meaning });
    renderPendingList();
    document.getElementById('single-kanji-input').value = '';
    document.getElementById('single-reading-input').value = '';
    document.getElementById('single-meaning-input').value = '';
